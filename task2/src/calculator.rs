...
impl Calculator {
    ...
    pub fn addition(&mut self, x: i64, y: i64) -> Option<i64> {
        let a = Operation::new(x, y, OperationType::Addition);
        let b = a.operation_type.perform(x, y);
        if let Some(_n) = b {
            self.history.push(a);
        }
        b
    ...
    pub fn subtraction(&mut self, x: i64, y: i64) -> Option<i64> {
        let a = Operation::new(x, y, OperationType::Subtraction);
        let b = a.operation_type.perform(x, y);
        if let Some(_n) = b {
            self.history.push(a);
        }
    ...
    pub fn multiplication(&mut self, x: i64, y: i64) -> Option<i64> {
        let a = Operation::new(x, y, OperationType::Multiplication);
        let b = a.operation_type.perform(x, y);
        if let Some(_n) = b {
            self.history.push(a);
        }
    ...
    pub fn show_history(&self) -> String {
        let mut a = "".to_string();
        for (index, i) in self.history.iter().enumerate() {
            a = format!(
                "{a}{}: {} {} {} = {}\n",
                index,
                i.first_num,
                i.operation_type.get_sign(),
                i.second_num,
                i.operation_type.perform(i.first_num, i.second_num).unwrap()
            );
        }
        a
    }
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64> {
        let option = self.history.clone();
        if let Some(o) = option.get(operation_index) {
            self.history.push(o.clone());
            return o.operation_type.perform(o.first_num, o.second_num);
        }
        None
    }
