...
impl Rectangle {
    pub fn new(width: f64, height: f64) -> Result<Self, Error> {
        match (width, height) {
            (..0.0, _) => return Err(Error::InvalidWidth),
            (_, ..0.0) => return Err(Error::InvalidHeight),
            _ => return Ok(Self { width, height }),
        }
    }
...
impl Circle {
    pub fn new(radius: f64) -> Result<Self, Error> {
        match radius {
            ..0.0 => return Err(Error::InvalidRadius),
            _ => return Ok(Self { radius }),
        }
    }
